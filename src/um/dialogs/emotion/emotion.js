(function () {
  ...
  UM.registerWidget('emotion', {
    ...
    '<ul class="edui-emotion-Jtabnav edui-tab-nav" style="overflow: hidden;">' +
    '<li class="edui-tab-item"><a data-context=".edui-emotion-Jtab0" hideFocus="true" class="edui-tab-text">emoji</a></li>' +
    ...
    sourceData: {
      emotion: {
        tabNum: 1, // 切换面板数量
        SmilmgName: {'edui-emotion-Jtab0': ['ali_', 98]}, // 图片前缀名
        ...
        imageFolders: {'edui-emotion-Jtab0': 'ali/'}, // 图片对应文件夹路径
        ...
        SmileyInfor: {
          'edui-emotion-Jtab0': ['微笑', '害羞', '吐舌头', '偷笑', '爱慕', '大笑', '跳舞', '飞吻', '安慰', '抱抱', '加油', '胜利', '强', '亲亲', '花痴', '露齿笑', '查找', '呼叫', '算账', '财迷', '好主意', '鬼脸', '天使', '再见', '流口水', '享受', '色情狂', '呆若木鸡', '思考', '迷惑', '疑问', '没钱了', '无聊', '怀疑', '嘘', '小样', '摇头', '感冒', '尴尬', '傻笑', '不会吧', '无奈', '流汗', '凄凉', '困了', '晕', '忧伤', '委屈', '悲泣', '大哭', '痛哭', 'I服了U', '对不起', '再见', '皱眉', '好累', '吐', '背', '惊讶', '惊愕', '闭嘴', '欠扁', '鄙视你', '大怒', '生气', '财神', '学习雷锋', '恭喜发财', '小二', '老大', '邪恶', '单挑', 'CS', '隐形人', '炸弹', '惊声尖叫', '漂亮MM', '帅哥', '招财猫', '成交', '鼓掌', '握手', '红唇', '玫瑰', '残花', '爱心', '心碎', '钱', '购物', '礼物', '收邮件', '电话', '举杯庆祝', '时钟', '等待', '很晚了', '飞机', '支付宝']
    ...
    initEvent: function () {
      ...
      me.root().delegate('td', 'click', function (evt) {
        var $td = $(this),
          ...
          emotionShotCut = $td.data('shot-cut') || null
        if (realUrl) {
          me.insertSmiley(realUrl.replace(/'/g, '\''), emotionShotCut, evt)
        }
        return false
      })
      // 更新模板
    ...
    insertSmiley: function (url, shotcut, evt) {
      // var obj = {
      //   src: url
      // }
      // obj._src = obj.src
      editor.execCommand('inserthtml', shotcut)
      if (!evt.ctrlKey) {
        // 关闭预览
        this.previewBox.removeClass('edui-emotion-preview-left').hide()
    ...
    createTab: function (contentBoxId) {
      var faceVersion = '?v=1.1', // 版本号
        ...
        sUrl, realUrl, posflag, offset, infor, emojiName
      ...
      for (; i < imgNum;) {
        ...
        for (var j = 0; j < imgColNum; j++, i++) {
          ...
          if (faceImage) {
            ...
            emojiName = emotion.emojiName[i] // 获取表情名

            textHTML.push('<td  class="edui-emotion-' + tableCss + '" data-surl="' + sUrl + '" data-realurl="' + realUrl + '" data-posflag="' + posflag + '" data-shot-cut="' + emojiName + '" align="center">'
            )
            textHTML.push('<span>')
            textHTML.push('<img  style="width: ' + iWidth + 'px" src="' + realUrl + '"></img>')
            textHTML.push('</span>')
          } else {
