<template src="./template/custom.html">
</template>

<script>
  import { knowledgeList, addKnowledge, deleteKnowledge, addQuestion, editQuestion, deleteQuestion, initEditAnswer, editAnswer, saveEditAnswer, delAnswer, officalEnable, relatedProductList, getRegular, getErrorCode, labelList } from '../../service/index'
  ...
  export default {
    ...
    methods: {
      ...
      addKnowledge () {
        this.ui.showEditKnowledge = true
      },
      // 新建知识库
      knowledgeSave () {
        this.$Spin.show()
        this.ui.isAjax = true
        addKnowledge(this.knowledgeForm).then((res) => {
          ...
          this.ui.showEditKnowledge = false
          setTimeout(() => { this.getData() }, 300)
        })
      },
      // 新建/编辑问法
      editModal (id, kdgBaseId) {
        this.$Spin.show()
        this.questionForm.kdgBaseId = kdgBaseId
        if (id === '') {
          this.questionForm.quesId = ''
          this.questionForm.content = ''
        } else {
          editQuestion({gid: id}).then((res) => {
            this.$Spin.hide()
            if (res.code !== 1) {
              return false
            }
            this.questionForm.quesId = id
            this.questionForm.content = res.data.content
          })
        }
        this.ui.showEditQuestion = true
      },
      // 保存问法
      questionSave () {
        this.ui.isAjax = true
        ...
        addQuestion(this.questionForm).then((res) => {
          ...
          this.ui.showEditQuestion = false
          this.$Message.success(res.msg)
          setTimeout(() => { this.getData() }, 300)
        })
      },
      ...
      showConfirm (id, title, type, tagType) {
        console.log(id + ',' + title + ',' + type + ',' + tagType)
